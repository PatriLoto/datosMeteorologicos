...
mapaA2012 <- estaciones_meteo2%>% filter(pais=='Argentina'& (year(fechaGrafico))== 2011)%>% select(lat, lon,t_max, t_min, media, fechaGrafico)
mapaA2012
...
mundo
mapaRladies <- mundo +
  geom_point(aes(x = mapaA2012$lon, y = mapaA2012$lat,
                 # text = paste('precipitacion: ', precipitacion),
                 # '<br /> Miembros : ', miembros),
                 #'<br /> Creado : ', creacion),
                 size = mapaA2012$media), data = mapaCorrientes, colour = "#88398A", alpha = .5)  +     
...
#-----------------------------------------------------------------------------------
##grafico temperaturas maximas accesible
...
library(gganimate)
mapaRladies+
  transition_time(year(fechaGrafico)) +
  ease_aes('linear')+
